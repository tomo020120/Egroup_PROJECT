...
public class MySQLHistoryDao implements HistoryDao{
	...
	public List<AllBrowsingHistoryInfoBean> getProductHistory(String userId) {
		...
		try {
			...
			String sql = " SELECT history.itemId,product.name,product.price,pict.pictPath,history.clickDate FROM browsing_history_table AS history LEFT OUTER JOIN product_table AS product ON history.itemId=product.itemId LEFT OUTER JOIN item_pict_table AS pict ON history.itemId=pict.itemId WHERE userId=? and clickDate = (select max(clickdate) from browsing_history_table bh where userId = ? and bh.itemId = history.itemId) group by history.itemId order by clickDate desc";
