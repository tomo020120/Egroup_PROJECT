...
public class PurchseInsertCreditCardInfoCommand extends AbstractCommand {
			...
			if(edit.getSameCreditCardQuantity(cardNo, userId) == 0) { // 同一のクレジットカード既に登録されていたらエラー処理
				if(CreditCardSecurityCodeAuthentication.isCorrectSecurityCode(cardNo, securityCode)) { // セキュリティコード認証
					if(edit.addCreditCardInfo(creditCardBean)) {
				    	...
				    	ConnectionManager.getInstance().beginTransaction();
						String cardId=dao.getCardId(userId,cardNo);

						url += "orderConfirmation?creditCardInfoId="+cardId;
						System.out.println("カード番号"+cardId);
					...
					//認証エラーメッセージを格納
					url += "choiceCredit?message=セキュリティコードが違います。";
				...
				// 既に登録されていた時の処理
				url += "choiceCredit?message=そのカードはすでに登録されています。";
				ConnectionManager.getInstance().closeTransaction();
			}
